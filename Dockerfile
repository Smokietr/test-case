...
LABEL maintainer="Özgür Can Karagöz <ozgurcankaragoz@usa.com>"
...
RUN apt-get update && apt-get install -yqq --no-install-recommends libmagickwand-dev \
...
RUN mkdir -p \
  ...
  vendor \
  database \
  && chmod -R ug+rwx storage bootstrap/cache public vendor database
